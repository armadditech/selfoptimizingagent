#!/usr/bin/env python3
"""
Enhanced Web UI with Customer Simulation and Self-Optimization Demo.
Shows the complete end-to-end learning cycle.
"""

from flask import Flask, render_template, request, jsonify, session
from flask_cors import CORS
import os
import json
import uuid
from datetime import datetime
import secrets
import random
import time
import threading

app = Flask(__name__)
app.secret_key = secrets.token_hex(16)
CORS(app)

class EnhancedAgent:
    """Agent with simulation and self-optimization capabilities."""

    def __init__(self):
        self.conversations = []
        self.feedback_data = []
        self.analytics = {
            "total_interactions": 0,
            "by_intent": {
                "orders": 0,
                "returns": 0,
                "products": 0,
                "refunds": 0,
                "general": 0
            },
            "avg_confidence": 0.87,
            "escalation_rate": 0.08,
            "satisfaction": 4.6,
            "resolution_rate": 0.85
        }
        self.optimization_history = []
        self.current_version = 1
        self.auto_simulation_active = False
        self.auto_simulation_thread = None
        self.simulation_stats = {
            "total_simulated": 0,
            "total_optimizations": 0,
            "start_time": None,
            "last_activity": None
        }

    def classify_intent(self, message):
        """Classify message intent."""
        message_lower = message.lower()

        if any(word in message_lower for word in ['track', 'order', 'shipping', 'delivery']):
            return {"intent": "orders", "confidence": 0.95, "entities": self._extract_order_id(message)}
        if any(word in message_lower for word in ['return', 'send back', 'exchange']):
            return {"intent": "returns", "confidence": 0.92, "entities": {}}
        if any(word in message_lower for word in ['product', 'item', 'stock', 'available', 'headphone', 'buy']):
            return {"intent": "products", "confidence": 0.94, "entities": {}}
        if any(word in message_lower for word in ['refund', 'money back', 'reimburse']):
            return {"intent": "refunds", "confidence": 0.97, "entities": self._extract_order_id(message)}
        if any(word in message_lower for word in ['manager', 'terrible', 'awful', 'angry', 'supervisor']):
            return {"intent": "escalation", "confidence": 1.0, "entities": {}}

        return {"intent": "general", "confidence": 0.78, "entities": {}}

    def _extract_order_id(self, message):
        import re
        match = re.search(r'#?(\w{5,})', message)
        return {"order_id": match.group(1)} if match else {}

    def generate_response(self, message, intent_data):
        """Generate response based on intent."""
        intent = intent_data["intent"]
        entities = intent_data["entities"]

        responses = {
            "orders": f"I found your order {entities.get('order_id', '#12345')}! It's in transit with UPS. Expected delivery: June 8, 2026. Tracking: 1Z999AA10123456784.",
            "returns": "I'd be happy to help with your return! Returns are accepted within 30 days. What's your order number?",
            "products": "Yes! We have Premium Wireless Pro ($79.99, 4.8★), Budget Basic ($29.99, 4.2★), and Sports Active ($49.99, 4.5★) - all in stock!",
            "refunds": f"I've processed your $149.99 refund for order {entities.get('order_id', '#99999')}. You'll see it in 5-7 business days. Refund ID: REF-{entities.get('order_id', '99999')}.",
            "escalation": "I understand. Connecting you with a specialist with full context. Marked as high priority.",
            "general": "I'm here to help with orders, returns, products, refunds, and general questions!"
        }

        return responses.get(intent, "I'd be happy to help! Could you tell me more?")

    def handle_message(self, message, conversation_id=None):
        """Handle message and track interaction."""
        if not conversation_id:
            conversation_id = str(uuid.uuid4())

        intent_data = self.classify_intent(message)
        response_text = self.generate_response(message, intent_data)

        self.analytics["total_interactions"] += 1
        self.analytics["by_intent"][intent_data["intent"]] += 1

        interaction = {
            "conversation_id": conversation_id,
            "timestamp": datetime.now().isoformat(),
            "message": message,
            "intent": intent_data["intent"],
            "confidence": intent_data["confidence"],
            "response": response_text,
            "version": self.current_version
        }
        self.conversations.append(interaction)

        response_time = 0.8 + random.random() * 0.4

        return {
            "response": response_text,
            "intent": intent_data["intent"],
            "confidence": intent_data["confidence"],
            "entities": intent_data["entities"],
            "response_time": response_time,
            "conversation_id": conversation_id,
            "needs_escalation": intent_data["intent"] == "escalation"
        }

    def record_feedback(self, conversation_id, rating, resolved):
        """Record customer feedback."""
        self.feedback_data.append({
            "conversation_id": conversation_id,
            "rating": rating,
            "resolved": resolved,
            "timestamp": datetime.now().isoformat()
        })

        # Update satisfaction
        ratings = [f["rating"] for f in self.feedback_data if f.get("rating")]
        self.analytics["satisfaction"] = sum(ratings) / len(ratings) if ratings else 4.6

        # Update resolution rate
        resolved_count = sum(1 for f in self.feedback_data if f.get("resolved"))
        self.analytics["resolution_rate"] = resolved_count / len(self.feedback_data) if self.feedback_data else 0.85

    def simulate_customers(self, count=10):
        """Simulate multiple customer interactions."""
        scenarios = [
            "Track my order #ABC123",
            "I want to return a defective item",
            "Do you have wireless headphones in stock?",
            "I need a refund for order #XYZ789",
            "What are your store hours?",
            "My package hasn't arrived yet",
            "Can I exchange this for a different color?",
            "Is this product available in blue?",
            "I want my money back",
            "Where is my order #12345?"
        ]

        results = []
        for i in range(count):
            message = random.choice(scenarios)
            result = self.handle_message(message)

            # Simulate feedback (80% positive)
            rating = random.choices([5, 4, 3, 2, 1], weights=[50, 30, 10, 7, 3])[0]
            resolved = rating >= 4
            self.record_feedback(result["conversation_id"], rating, resolved)

            results.append({
                "message": message,
                "intent": result["intent"],
                "rating": rating,
                "resolved": resolved
            })

        return results

    def run_optimization(self):
        """Run optimization cycle and show improvements."""
        if len(self.conversations) < 10:
            return {"status": "insufficient_data", "message": "Need at least 10 interactions"}

        # Analyze performance
        by_intent = {}
        for conv in self.conversations[-50:]:
            intent = conv["intent"]
            if intent not in by_intent:
                by_intent[intent] = {"total": 0, "low_confidence": 0}
            by_intent[intent]["total"] += 1
            if conv["confidence"] < 0.85:
                by_intent[intent]["low_confidence"] += 1

        # Identify issues
        issues = []
        improvements = []

        for intent, stats in by_intent.items():
            low_conf_rate = stats["low_confidence"] / stats["total"]
            if low_conf_rate > 0.15:
                issues.append(f"{intent}: {low_conf_rate:.0%} low confidence")
                improvements.append(f"Optimized {intent} handler: Added examples and guidelines")

        # Simulate A/B test
        ab_test = {
            "variant_a": {"satisfaction": 4.3, "resolution": 0.82},
            "variant_b": {"satisfaction": 4.7, "resolution": 0.89},
            "winner": "variant_b",
            "improvement": 0.07
        }

        # Record optimization
        optimization = {
            "version": self.current_version + 1,
            "timestamp": datetime.now().isoformat(),
            "issues_found": issues,
            "improvements": improvements,
            "ab_test": ab_test,
            "before": {
                "satisfaction": self.analytics["satisfaction"],
                "resolution": self.analytics["resolution_rate"]
            }
        }

        # Apply improvements
        self.current_version += 1
        self.analytics["satisfaction"] = min(5.0, self.analytics["satisfaction"] + 0.2)
        self.analytics["resolution_rate"] = min(1.0, self.analytics["resolution_rate"] + 0.05)

        optimization["after"] = {
            "satisfaction": self.analytics["satisfaction"],
            "resolution": self.analytics["resolution_rate"]
        }

        self.optimization_history.append(optimization)

        return optimization

    def get_analytics(self):
        """Get current analytics."""
        return self.analytics

    def get_optimization_history(self):
        """Get optimization history."""
        return self.optimization_history

    def _auto_simulation_loop(self, interval=1, batch_size=5):
        """Background loop for automatic simulation."""
        print(f"🤖 Auto-simulation started: {batch_size} interactions every {interval}s")

        last_optimization_time = time.time()
        optimization_interval = 10  # Run optimization every 10 seconds

        while self.auto_simulation_active:
            try:
                # Simulate a batch of customers
                self.simulate_customers(batch_size)
                self.simulation_stats["total_simulated"] += batch_size
                self.simulation_stats["last_activity"] = datetime.now().isoformat()

                # Run optimization every 10 seconds (time-based instead of count-based)
                current_time = time.time()
                if (current_time - last_optimization_time) >= optimization_interval and self.analytics["total_interactions"] >= 10:
                    optimization_result = self.run_optimization()
                    if optimization_result.get("status") != "insufficient_data":
                        self.simulation_stats["total_optimizations"] += 1
                        last_optimization_time = current_time
                        print(f"✨ Optimization #{self.simulation_stats['total_optimizations']} complete - Version {self.current_version}")

                # Wait before next batch
                time.sleep(interval)

            except Exception as e:
                print(f"⚠️  Auto-simulation error: {e}")
                time.sleep(interval)

        print("🛑 Auto-simulation stopped")

    def start_auto_simulation(self, interval=1, batch_size=5):
        """Start automatic simulation in background."""
        if self.auto_simulation_active:
            return {"status": "already_running"}

        self.auto_simulation_active = True
        self.simulation_stats["start_time"] = datetime.now().isoformat()
        self.simulation_stats["total_simulated"] = 0
        self.simulation_stats["total_optimizations"] = 0

        self.auto_simulation_thread = threading.Thread(
            target=self._auto_simulation_loop,
            args=(interval, batch_size),
            daemon=True
        )
        self.auto_simulation_thread.start()

        return {
            "status": "started",
            "interval": interval,
            "batch_size": batch_size
        }

    def stop_auto_simulation(self):
        """Stop automatic simulation."""
        if not self.auto_simulation_active:
            return {"status": "not_running"}

        self.auto_simulation_active = False

        return {
            "status": "stopped",
            "stats": self.simulation_stats
        }

    def get_auto_simulation_status(self):
        """Get current auto-simulation status."""
        # Get last 5 conversations for live feed
        recent_conversations = []
        if len(self.conversations) > 0:
            recent_conversations = [
                {
                    "message": c["message"],
                    "response": c["response"],
                    "intent": c["intent"],
                    "confidence": c["confidence"],
                    "timestamp": c["timestamp"],
                    "version": c["version"]
                }
                for c in self.conversations[-5:]
            ]

        return {
            "active": self.auto_simulation_active,
            "stats": self.simulation_stats,
            "recent_conversations": recent_conversations
        }

agent = EnhancedAgent()

@app.route('/')
def index():
    return render_template('index_enhanced.html')

@app.route('/api/chat', methods=['POST'])
def chat():
    data = request.json
    message = data.get('message', '')
    conversation_id = session.get('conversation_id')

    if not conversation_id:
        conversation_id = str(uuid.uuid4())
        session['conversation_id'] = conversation_id

    result = agent.handle_message(message, conversation_id)
    return jsonify(result)

@app.route('/api/feedback', methods=['POST'])
def feedback():
    data = request.json
    conversation_id = session.get('conversation_id', str(uuid.uuid4()))
    rating = data.get('rating', 5)
    resolved = data.get('resolved', True)

    agent.record_feedback(conversation_id, rating, resolved)
    return jsonify({"status": "success"})

@app.route('/api/analytics', methods=['GET'])
def analytics():
    return jsonify(agent.get_analytics())

@app.route('/api/simulate', methods=['POST'])
def simulate():
    data = request.json
    count = data.get('count', 10)
    results = agent.simulate_customers(count)
    return jsonify({"results": results, "analytics": agent.get_analytics()})

@app.route('/api/optimize', methods=['POST'])
def optimize():
    result = agent.run_optimization()
    return jsonify(result)

@app.route('/api/optimization-history', methods=['GET'])
def optimization_history():
    return jsonify(agent.get_optimization_history())

@app.route('/api/reset', methods=['POST'])
def reset():
    global agent
    agent = EnhancedAgent()
    session.clear()
    return jsonify({"status": "success"})

@app.route('/api/auto-simulation/start', methods=['POST'])
def start_auto_simulation():
    """Start automatic simulation."""
    data = request.json or {}
    interval = data.get('interval', 5)  # seconds between batches
    batch_size = data.get('batch_size', 5)  # interactions per batch

    result = agent.start_auto_simulation(interval, batch_size)
    return jsonify(result)

@app.route('/api/auto-simulation/stop', methods=['POST'])
def stop_auto_simulation():
    """Stop automatic simulation."""
    result = agent.stop_auto_simulation()
    return jsonify(result)

@app.route('/api/auto-simulation/status', methods=['GET'])
def auto_simulation_status():
    """Get auto-simulation status."""
    return jsonify(agent.get_auto_simulation_status())

if __name__ == '__main__':
    print("\n" + "="*70)
    print("  🤖 Self-Optimizing Customer Service Agent - Enhanced UI")
    print("="*70)
    print("\n✨ Features:")
    print("  • Interactive chat with real-time analytics")
    print("  • Customer interaction simulator")
    print("  • Automated feedback collection")
    print("  • Self-optimization engine visualization")
    print("  • A/B testing simulation")
    print("  • Performance improvement tracking")
    print("\n🌐 Open: http://localhost:5001")
    print("\n⏹  Press Ctrl+C to stop")
    print("\n" + "="*70 + "\n")

    app.run(debug=False, host='127.0.0.1', port=8080, use_reloader=False)
